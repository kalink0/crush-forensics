...
- Fixed a raw disk image opening only as hex when its name didn't end in `.img`/`.dd`/`.raw` (e.g. the `.bin` many acquisition tools write, or a filesystem dump with no extension): disk images are now recognised by their content — partition table or bare filesystem — whatever they're called. EWF (`.E01`) acquisitions and split `.001` sets still need their extensions, since their segments are found by name. The large-file prompt now also offers **Open in New Window** for any file, not just known archive/image extensions, so a large nested `.bin` image can be browsed as a source.
...
- Opening a file that could exhaust free memory (Open, or any Open as… mode) now asks first: open anyway (only offered while it can plausibly fit), open in a new window, export, or cancel. Large reads, hashes and hex searches run behind a wait dialog so the window stays responsive.
...
- Image Viewer now detects embedded C2PA (Content Credentials) manifests — JPEG, PNG, GIF, WebP, TIFF, HEIC/HEIF/AVIF, and box-form JPEG XL, matching EXIF's format coverage — showing the generator software, claimed actions with their software agent(s), the IPTC Digital Source Type (the actual "AI-generated/-edited" signal, shown as its official IPTC name), ingredients (prior assets in the provenance chain), and the claim signature's leaf certificate identity in the Properties panel — structure only, not a cryptographic trust verification. Every format's embedding layout (JUMBF box structure, JPEG APP11 fragmentation, GIF Application Extension, RIFF/WebP chunk, TIFF tag, BMFF `uuid` box) was verified against real sample files and the reference implementation published by the C2PA org, not just the written spec. A second, independent path reads the IPTC Digital Source Type directly from XMP when present — container-agnostic, so it also catches images some tools (e.g. Google/Gemini) mark this way without embedding a full C2PA manifest at all. Addresses [#92](https://github.com/kalink0/crush-forensics/issues/92), reported by [@JamesHabben](https://github.com/JamesHabben).
...
- SQLite Table Viewer now has the same **Show Hex** pane, extended to per-cell precision: selecting a cell highlights its row and that specific column's on-disk bytes; a row whose current version is only in a not-yet-checkpointed `-wal` frame switches the pane to that file instead of the base file's stale bytes.
- Realm DB Table Viewer's **Show Hex** pane now highlights real bytes in the actual `.realm` file (both modern and legacy on-disk formats), not a synthetic SQLite copy used only for querying.
...
- SQLite Table Viewer's WAL Frames tab now supports the embedded Show Hex pane too: selecting a frame highlights its exact header+page bytes in the `-wal` file, and clicking a byte in that range selects the matching frame back. A superseded/uncommitted row injected into a normal table's own view by the "Show WAL history" toggle gets the same treatment, at full cell/column precision (not just the whole frame), since it has no rowid a CellLocator could resolve either. Freeblocks and Unallocated Space rows now support it too, at exact cell precision (each row already knows its own on-disk offset and size).
- SQLite Table Viewer now has a File Structure tab, showing the database file's physical page/cell/header layout (allocation status, cell pointers, freeblocks, unallocated space) with the same Show Hex byte-provenance as table cells — selecting a structure item highlights its exact bytes across base/WAL files and vice versa. Pages load their row/column detail on first expand rather than all at once, so opening a database with many rows stays responsive. ([@JamesHabben](https://github.com/JamesHabben), [#86](https://github.com/kalink0/crush-forensics/pull/86))
...
- Fixed the WAL Frames tab and SQLite Table Viewer's WAL-row injection misparsing any WAL frame targeting page 1 as "not a leaf page" instead of decoding it — page 1's b-tree header starts at byte 100, not 0, which nothing in the codebase accounted for until now.
- Fixed both of those same WAL-frame decode paths rendering any value that spilled onto an overflow page as the `<OVERFLOW>` sentinel instead of its real content, even though the same overflow-chasing already backs the Show Hex pane's byte lookup for regular table rows.
- Fixed exports failing on Windows when a source name (from a non-Windows acquisition) contained a character Windows filesystems reject (`: " < > | ? *`), a reserved device name (`CON`, `COM1`, ...), or a trailing dot/space — sanitization previously only replaced path separators, and the filtered/multi-file export path didn't sanitize at all. Every renamed entry is now logged and recorded in a `crush-export-renames.txt` sidecar next to the export for forensic traceability back to the original name.
- Fixed the Log pane showing every open window's log messages instead of just its own — all `MainWindow` instances shared one process-wide logger, so N open windows meant N handlers all receiving every record. Every source of log output is now attributed to the window it actually belongs to: the window's own code, its load/export/logarchive worker threads, a parser invoked while opening or re-opening a file (including from a BLOB cell's "Open as new tab"), the filesystem tree's background type pre-scan, and Multi-Log Studio's loader/sort workers. Each window's log file still records everything, as before. ([#87](https://github.com/kalink0/crush-forensics/issues/87))
...
- SQLite Table Viewer's "Open as new tab" is now a submenu (**Auto-detect** / **Hex** / **Text** / **Protobuf**) instead of a single auto-detected action, and each blob cell — including individual rows from an ad-hoc SQL query result — gets its own tab identity, so opening several cells no longer risks one tab silently showing another's data. Tab labels now show just the filename; the full path, display format, and (for these cells) source table or full query text, column, row, and originating database file are shown in the tab tooltip and Properties panel. ([@JamesHabben](https://github.com/JamesHabben), [#72](https://github.com/kalink0/crush-forensics/pull/72), [#74](https://github.com/kalink0/crush-forensics/pull/74), [#77](https://github.com/kalink0/crush-forensics/pull/77))
- Added a `--focus REL_PATH` CLI flag, for use alongside a single `PATH`/`--open` target: opens that one file/folder/archive and immediately selects and opens a specific file inside it, instead of just showing the tree. Meant for other tools invoking Crush with evidence already queued up and a specific file to jump straight to.
