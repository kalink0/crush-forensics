...
- Protobuf Viewer's schema-less decode tree now has an Export… toolbar button (whole tree, as text or JSON) and a right-click "Export Subtree as Text/JSON…" action for a single field and its children. The JSON export always contains a bytes field's complete value, not the tree's 64-byte display preview. Addresses [#64](https://github.com/kalink0/crush-forensics/issues/64).
