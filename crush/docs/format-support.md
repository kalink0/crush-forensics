...
- Embedded Show Hex byte-provenance is not yet extended to every viewer — done so far: Protobuf, Plist/XML Tree, SQLite Table (cells, WAL Frames, File Structure), SEGB/Biome, and Realm (Tables, File Structure). Not yet covered: plist's binary-vs-XML split (binary has an on-disk offset table to reuse; XML doesn't), ABX, LevelDB, MMKV, the Blob Inspector's protobuf-schema path, and text-position-based viewers (JSON, XML, Multi-Log Studio).
