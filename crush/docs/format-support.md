...
- The Tables tab's embedded **Show Hex** pane highlights real bytes in the actual `.realm` file, not the temporary SQLite copy above — precomputed per cell while the file is decoded, for both the modern Cluster format and the legacy pre-Cluster layout.
- A **File Structure** tab shows the file's own physical array/reference-graph layout (file header, streaming-form footer where applicable, Group top array and its children, free list, and per-table Spec/row-storage), independent of the decoded Tables view, with the same Show Hex byte-provenance — selecting a structure item highlights its exact bytes and vice versa. Pre-Cluster tables' row storage (a separate top-level B+-tree per column, rather than one shared Cluster tree) is walked and shown the same way, under a "Column B+-Trees" branch instead of "ClusterTree".
...
- Detects JSON Lines, Android logcat, Syslog (RFC 3164), and generic timestamped/plain-text logs. Text logs have no format marker, so this is a heuristic: every candidate is scored against every line, the first to reach its threshold (JSON Lines ≥ 60 % of lines, logcat ≥ 50 %, Syslog ≥ 50 %, Generic ≥ 2 lines) is used, and the format label says "(heuristic)". The ⓘ next to a source shows all candidate scores, the rule, lines that didn't match, timestamp notes and encoding problems. **Format → Re-parse … as** overrides the choice.
- Syslog, generic and plain-text logs have no level field: the level is guessed from a keyword in the message and shown as "(guessed)".
- Timestamps show only what the log recorded: a time without a zone is shown as recorded with "(no zone)" and is never converted by **Display TZ**; a log without a year (Syslog RFC 3164, logcat) shows the year as `????`. Offsets in a timestamp (`+02:00`, `Z`) are applied.
...
- Syslog (RFC 3164) and logcat record no year: those entries sort on a placeholder year, so their order against other sources is not meaningful. Zone-less times sort and filter as if they were UTC.
