...
**Cellebrite UFDR (10.x)** — browse the original device's file/folder tree from a Physical Analyzer report container, reconstructed from the container's embedded PostgreSQL dump rather than its own internal, type-bucketed storage layout, with Cellebrite's own recorded MD5/SHA-256 shown per file. Filesystem browsing only — Cellebrite's other forensic tables (contacts, calls, chats, etc.) are not decoded.
